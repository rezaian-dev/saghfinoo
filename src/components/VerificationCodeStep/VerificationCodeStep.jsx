...
const VerificationCodeStep = memo(({ 
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.key === 'Enter' && state.otp.length === 5 && !state.otpError && !state.isOtpCorrect) {
        e.preventDefault();
  ...
  return (
    <>
      ...
      <div className="modal__verification-subtitle">
        <span className="modal__verification-subtitle__text">
          کد ارسال شده به شماره {userPhoneNumber} را وارد کنید
        </span>
        <button onClick={handleBack} className="modal__verification-subtitle__edit">
          ویرایش شماره موبایل
        </button>
      </div>

      {/* 📝 OTP input form */}
      <form onSubmit={handleSubmit(onSubmitVerification)} className="modal__verification-form">
        {/* 🔢 OTP input fields */}
        ...
        <div className="modal__verification-form__timer">
          {state.isResendActive ? (
            // 🔄 Resend code button
            <span className="modal__verification-form__timer-resend" onClick={handleResendCode}>
              ارسال مجدد کد
            </span>
          ) : (
            <>
              {/* ⏳ Countdown timer */}
              <div className="modal__verification-form__timer-countdown">
                <Clock size="16" color="#868686" />
                <span className="verification-timer__time">{formatTime(state.timeLeft)}</span>
              ...
              <span className={clsx(
                ...
                state.showCodeUser && "modal__verification-form__user-code--visible"
              )}>
        ...
        <button
          ref={submitModal}
          type="submit"
          disabled={state.otp.length !== 5 || state.isOtpCorrect || state.otpError}
          className={clsx(
            "modal__verification-form__submit",
            state.otp.length === 5 && !state.otpError && "modal__verification-form__submit--active"
          )}
        >
          تایید
        </button>
